...
import { HoverSample, AIEvaluation, AIEvalDetail, HarnessReport, Regression } from './types';
...
export class AIEvaluator {
  ...
  async evaluateHoverQuality(samples: HoverSample[]): Promise<AIEvaluation> {
    ...
    if (toEvaluate.length === 0) {
      return { hover_quality_avg: 0, diagnostic_accuracy: 0, samples_evaluated: 0, details: [] };
    }
    ...
    return {
      ...
      diagnostic_accuracy: 0, // Filled separately
